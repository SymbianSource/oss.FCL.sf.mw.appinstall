...
std::string Util::Base64Encode( const std::string& aBinaryData )
	{
	BIO *b64 = BIO_new(BIO_f_base64());
	BIO *mem = BIO_new(BIO_s_mem());
	mem = BIO_push(b64, mem);
	BIO_write(mem, aBinaryData.c_str(), aBinaryData.length());
	BIO_flush(b64);

	BUF_MEM *bptr;
	BIO_get_mem_ptr(mem, &bptr);
	std::string encodedString(bptr->data, bptr->length); 
