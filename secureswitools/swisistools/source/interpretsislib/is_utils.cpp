...
#ifdef _WIN32
#include "utils_win32.cpp"
#else
#include "utils_posix.cpp"
#endif

namespace 
{
	
	wchar_t Convert2Wide(char x)
	{
		return 0x00FF & x;
	}
	
	char Convert2Narrow(wchar_t x)
	{
		return x & 0x00FF;
	}
};
...
{
	aOut.resize(aStr.size());
	std::transform(aStr.begin(), aStr.end(), aOut.begin(), Convert2Narrow);
	return aOut;
}
...
std::string Ucs2ToUtf8(const std::wstring& aStr)
{
	...
	return Ucs2ToUtf8(aStr,str);
}

std::wstring& Utf8ToUcs2(const std::string& aStr, std::wstring& aOut)
{
	aOut.resize(aStr.size());
	std::transform(aStr.begin(), aStr.end(), aOut.begin(), Convert2Wide);
	return aOut;
}
...
void ConvertToLocalPath( std::wstring& aIn, const std::wstring& aDrivePath )
{
    ...
    //
    // Otherwise, we'll just prepend the PC's path, leaving
    // the data in aIn intact.
...
// generated from v4.1 of Unicode Consortium's CaseFolding.txt
// Only common and special conversions (C+S) are done.
TPair caseFoldTable[] = {
	...
	{ 0x0241, 0x0294 }, // # LATIN CAPITAL LETTER GLOTTAL STOP
...
struct Compare
{
	typedef wchar_t first_argument_type; 
	typedef TPair second_argument_type; 
	typedef bool result_type; 

	bool operator()(const TPair& aP, wchar_t aC) const
	{
		return aP.first < aC;
	}
};

wchar_t Fold(wchar_t aC) 
{
	static const size_t s = sizeof(caseFoldTable)/sizeof(TPair);
	
	TPair* x = std::lower_bound(
			caseFoldTable, caseFoldTable+s, 
			aC, Compare());

	if (x != caseFoldTable+s && x->first == aC )
	{
		return x->second;
	}
	return aC;
...
int ReadSecurityInfo( SBinarySecurityInfo& aInfo, const std::wstring aFileName )
{
    ...
    ConvertWideCharToMultiByte(aFileName.c_str(), -1 , fileName , PATHMAX);

    // GetSecurityInfo() has some rather innapropriate side effects when called
    ...
    if  (_dup2(_fileno(nulStdErr),2) == -1)
	{
        throw "Failed in redirection operation";
	}

    const TInt err = GetSecurityInfo( fileName, aInfo );
    fflush(stderr);
    _dup2(oldStdErr , 2);
    fclose(nulStdErr);
	// close duplicate handle, _dup2 will copy the handle values already
	// handle leak will happen, if not closed, see DEF115968 for more info. 
	close(oldStdErr);
    return err;
}

void Utils::GetEquivalentLanguageList(CSISLanguage::TLanguage aLang, TLanguagePath& aEquivalents) 
	{
	aEquivalents[0] = aLang;
	aEquivalents[1] = CSISLanguage::ELangNone;
	const TInt len = sizeof(KEquivalentLists) / sizeof(KEquivalentLists[0]);
	const TInt receiverLen = sizeof(aEquivalents) / sizeof(aEquivalents[0]);
	for (TInt i = 0; i < len; ++i)
		{
		const CSISLanguage::TLanguage *ptr = KEquivalentLists[i];
		if (ptr[0] == aLang)
			{
			TInt index = 1;
			while ((CSISLanguage::ELangNone != *ptr) && (index < receiverLen - 1))
				{
				aEquivalents[index++] = (CSISLanguage::TLanguage)*(++ptr);
				}
