...
sub FindCerts
{
	...
	if (($File::Find::dir !~ m/\/switestfw\/testcertificates/) && ($File::Find::dir !~ m/\/rootcerts/) && ($File::Find::dir !~ m/\/testframework\/testcertificates/))
	{	
...
if ($count eq 0)
{
	...
	foreach $certificatefile (@Certificates)
		{
		$certificatefile =~ s/\//\\/g;
		print (SCANLOG "Certificate: $certificatefile is in an invalid location. Should be moved to ......\\security\\installtestframework\\testcertificates\\...\n");
