...
static int readBlockIntoCache(crashFile *pFile, int blk){
  if( blk>=pFile->nBlk ){
    ...
    pFile->apBlk = (u8 **)crash_realloc(pFile->apBlk, n * sizeof(u8*));
...
*/
static void initFile(OsFile **pId, char const *zName, OsFile *pBase){
  crashFile *pFile = crash_malloc(sizeof(crashFile) + strlen(zName)+1);
  memset(pFile, 0, sizeof(crashFile) + strlen(zName)+1);
